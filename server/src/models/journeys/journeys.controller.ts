...
import { Journey, JourneySetting } from '@types';
...
export class JourneysController {
    ...
    @Get(':id')
    find(@Param('id') journeyId: string) : Promise<Journey | null>{
        this.logger.debug(`Requesting journey with id: ${journeyId}`);
        return this.journeysService.findById(journeyId);
    }

    @Put(':id/settings')
    update(@Param('id') journeyId: string, @Auth() auth, @Body() data: JourneySetting) : Promise<Journey | null> {
